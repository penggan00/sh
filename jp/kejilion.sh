...
list_forwarding_services() {
	...
	# 打印表头
	printf "%-20s %-25s %-30s %-10s\n" "服务名称" "イントラネットアドレス" "外部ネットワークアドレス" "协议"
		...
		while true; do
			...
			case "$perm_choice" in
				...
				2)
					...
					;;
				3)
					echo "⚠️ フルオープン モードは実行承認をオフにし、権限昇格と bash を有効にし、信頼できるシングルユーザー環境でのみ推奨されます。"
